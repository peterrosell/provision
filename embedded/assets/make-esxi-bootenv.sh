...
#  an ESXi install ISO.  It is limited in that it only tweaks the
#  minimal amount of templates necessary to create the bootenv.
#
...
case $(uname -s) in
  Darwin) hdiutil attach $ISO -readonly -mountpoint $ISO_MNT
    ...
    ;;
  Linux) mouunt -o ro,loop $ISO $ISO_MNT
